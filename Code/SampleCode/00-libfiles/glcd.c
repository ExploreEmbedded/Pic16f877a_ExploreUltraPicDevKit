...
void GLCD_Init()
{
    ...
    GlcdCtrlBusDirnReg &= ~((1<<RS)|(1<<RW)|(1<<EN)|(1<<CS1)|(1<<CS2)); // Configure the Ctrl pins as output
...
void GLCD_SetCursor(uint8_t lineNumber,uint8_t CursorPosition)
{
	...
	if((lineNumber <= C_GlcdLastLine_U8) && (CursorPosition <= 128))
	{
...
{
	glcd_BusyCheck();
	GlcdDataBus = dat ^ GLCD.Invertdisplay;
...
void glcd_BusyCheck()
{	
#ifdef BUSY_CHECK_EN            //Perform Busy check if LCD_RW pin is used
	uint8_t busyflag;

	util_UpdateBit(GlcdDataBus,BUSY_FLAG,C_PinInput_U8); // Configure busy pin as input
	GlcdClearBit(RS);           // Select the Command Register by pulling RS LOW
	...
	do
	{
		...
		GlcdSetBit(EN);
		DELAY_us(10);
		busyflag = util_GetBitStatus(GlcdDataBus,BUSY_FLAG);
	}while(busyflag);
#else
